...
The compact bottom-right music player loads the supplied Outer Wilds YouTube playlist with custom previous/play-pause/next, stop, and volume controls and reports the active title plus a readable elapsed/total time centered in the remaining control-row space when available. Stop rewinds to the beginning; the volume control toggles mute and reveals a volume slider on hover or keyboard focus, which remains open for two seconds after the pointer leaves. While music plays, the Sun makes a subtle single pulse on a slightly randomized 0.6–0.72 second interval; alternating pulse animations preserve the corona’s continuous motion and the effect respects reduced-motion preferences. All player icons are CSS-drawn rather than font glyphs for consistent rendering. The information panel is raised to leave a visible gap above it. The Settings menu persists an Enable autoplay on load option; otherwise it loads paused. Previous restarts after five seconds and moves to the preceding track within the first five seconds. It remains full-width on mobile for touch usability.
